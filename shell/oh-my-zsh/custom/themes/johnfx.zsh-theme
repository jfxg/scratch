function theme_precmd {
  ...
  if (( pwdsize > TERMWIDTH - 8 )); then
    (( PR_PWDLEN = TERMWIDTH - 8 ))
  fi
}
...
# display exitcode inline when > 0
return_code="%(?..%{$fg[red]%}%? ↵ %{$reset_color%})"

# Finally, the prompt.
PROMPT='${PR_SET_CHARSET}${PR_STITLE}${(e)PR_TITLEBAR}\
${PR_ORANGE}${PR_ULCORNER}${PR_HBAR}(\
${PR_YELLOW}%D{%Y-%m-%d %H:%M:%S} \
${PR_ORANGE}%(!.%SROOT%s.%n)@${PR_GREEN}%m\
${PR_ORANGE}) $return_code\

${PR_ORANGE}${PR_LLCORNER}${PR_HBAR}(\
${PR_GREEN}%${PR_PWDLEN}<...<%~%<<\
${PR_LIGHT_BLUE}%{$reset_color%}$(git_prompt_info)$(git_prompt_status)\
${PR_ORANGE})${PR_HBAR}>${PR_NO_COLOUR} '

RPROMPT=''

PS2='${PR_ORANGE}${PR_HBAR}\
${PR_BLUE}${PR_HBAR}(\
